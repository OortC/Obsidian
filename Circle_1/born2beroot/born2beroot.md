...
- 가상머신이란
	...
	- 하나의 물리적인 컴퓨터에서 여러 독립적인 가상 환경을 생성하여 다양한 운영 체제나 소프트웨어를 실행하고 관리함, 주로 개발 및 테스트 목적으로 사용
- 가상머신 작동방식
	1. 가상머신 생성
		- 가상머신은 가상화 소프트웨어에 의해 모의된 하드웨어 자원을 할당 받음
		- 이때 프로세서, 메모리, 디스크 공간을 할당받음
	2. 게스트 운영체제 설치
		- 운영 체제는 가상머신 내에서만 실행되며, 호스트 시스템의 운영체제와 독립적으로 동작
	3. 레어어, 자원 할당 및 스케쥴링
		- 가상화 소프트웨어는 가상머신과 하드웨어 간의 중간에 위치하여 가상화 레이어를 형성하고, 호스트 시스템의 물리적인 자원을 가상머신에게 할당함
		- 레이어는 가상머신이 하드웨어 자원을 직접 사용하지 않고, 호스트 시스템과의 상호 작용하도록 관리
		- 스케쥴링 알고리즘을 사용해 자원을 효율적으로 분배함
- centOS, Debian 차이점
	- centOS 
...
### 5. sudo
- sudo란?
	- 사용자의 특정 명령을 관리자 권한으로 실행할 수 있게 해주는 도구
...
### 6. UFW
- 방화벽
	- 네트워크에서 불법한 액세스를 차단하고 허가하지 않은 네트워크 트래픽으로부터 시스템을 보호하기 위한 보안 장치 또는 소프트웨어
	- 방화벽은 컴퓨터, 네트워크, 시스템의 중요한 부분으로 작동하여 허가 되지 않은 액세스를 차단 / 제어하고, 허용된 트래픽만 통과시키도록 구성
- UFW란?
